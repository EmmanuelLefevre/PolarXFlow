...
LEFT JOIN {{ ref('dim_commits') }} commits ON c.contributor_id = commits.author_name
LEFT JOIN {{ ref('dim_pull_requests') }} pr ON c.contributor_id = pr.author_name
LEFT JOIN {{ ref('dim_tags') }} tags ON commits.commit_id = tags.commit_id
LEFT JOIN {{ ref('dim_contents') }} contents ON c.contributor_id = contents.file_url
